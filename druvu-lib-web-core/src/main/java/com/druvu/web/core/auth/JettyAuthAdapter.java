 ...
 *
 * <p>Internally maps permissions to Jetty roles (1:1) so that Jetty's role-based security model works transparently
 * with the permissions-only API.
 *
 * @author Deniss Larka
 */
public class JettyAuthAdapter implements Authenticator.Configuration {
    ...
    private static LoginService createLoginService(AuthConfig config) {
        ...
        throw new UnsupportedOperationException("Custom UserStore implementations require a LoginService adapter. "
