 ...
 *
 * @author Deniss Larka on 21 September 2023
 */
@Slf4j
public class WebSocketCreator implements JettyWebSocketCreator {
    ...
    public Object createWebSocket(JettyServerUpgradeRequest upRequest, JettyServerUpgradeResponse upResponse) {
        ...
        if (callOpt.isPresent()) {
            ...
            final Set<WebSocketHandler.Session> openedSessions =
                    ContextVars.socketSessions(upRequest.getHttpServletRequest().getServletContext());
            return create(
                    HandlerUtils.handler(
                            call.request().globalAttributes(), call.request().mainPath()),
                    call.request(),
                    openedSessions);
        }
        try {
    ...
    public Session.Listener create(
            WebSocketHandler handler, HttpRequest request, Set<WebSocketHandler.Session> openedSessions) {
