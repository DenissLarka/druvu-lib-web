...
/** Tokenizer for PHP content. Parses content character-by-character to identify PHP tags and text content. */
public class PhpTokenizer {
            ...
            if (ch == '<') {
                ...
                if (position + 1 < length) {
                    ...
                    if (next == '?'
                            ...
                            && (Character.isWhitespace(content.charAt(position + 5))
                                    || content.charAt(position + 5) == '?')) {
    ...
    /** Parses a PHP tag: <?php ... ?> Determines if it's an echo, include/require, or generic PHP code */
    ...
    /**
     * Checks if the content at the current position matches a keyword, followed by a non-identifier character
     * (whitespace, parenthesis, quote).
     */
    ...
    /**
     * Parses the expression part of a PHP statement (echo, include, require). Extracts everything between the keyword
     * and the closing ?>, stripping trailing semicolons and optional outer parentheses.
     */
