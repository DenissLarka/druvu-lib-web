 ...
 *
 * <p>This implementation: - Uses PhpTokenizer for parsing - Uses PhpExpr for evaluation - Supports multiple PHP tag
 * types: <?php echo ... ?> and <?= ... ?> - Supports include/require/include_once/require_once statements
 */
public class TokenBasedPhpProcessor {
            ...
            switch (token.getType()) {
                ...
                case PHP_CODE:
                    ...
                    throw new PhpProcessingException(
                            "Generic PHP code blocks are not yet supported at position " + token.getStartPosition());

                default:
                    throw new PhpProcessingException("Unknown token type: " + token.getType());
            }
        }
    ...
    private String processInclude(PhpToken token, PhpContext context) {
        boolean isRequire =
                token.getType() == PhpToken.Type.PHP_REQUIRE || token.getType() == PhpToken.Type.PHP_REQUIRE_ONCE;
        boolean isOnce =
                token.getType() == PhpToken.Type.PHP_INCLUDE_ONCE || token.getType() == PhpToken.Type.PHP_REQUIRE_ONCE;
    ...
    /**
     * Resolves an include path relative to the current file's directory. Absolute paths (starting with /) are used
     * as-is. Relative paths are resolved against the directory of the current file.
     *
