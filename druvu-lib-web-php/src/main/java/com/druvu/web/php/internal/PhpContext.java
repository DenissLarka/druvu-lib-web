...
/**
 * Context object that holds request and response objects for PHP processing. This allows tag processors to access
 * servlet context when needed.
 *
 * @author Deniss Larka
 */
public class PhpContext {
    ...
    /**
     * Creates a derived context for processing an included file. Shares the same includedFiles set and resourceLoader,
     * but tracks a new currentPath and incremented include depth.
     *
    ...
    public PhpContext forInclude(String includedPath) {
        ...
        if (newDepth > MAX_INCLUDE_DEPTH) {
            throw new PhpProcessingException("Maximum include depth exceeded (" + MAX_INCLUDE_DEPTH + ")");
        }
        return new PhpContext(
                request, response, includedPath, includedFiles, resourceLoader, functionRegistry, newDepth);
    }
